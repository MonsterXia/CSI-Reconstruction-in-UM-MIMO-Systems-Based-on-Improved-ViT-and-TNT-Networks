...
def main():
    ...
    if os.path.isfile(file_path):
        ...
        model = ImageTransformer(img_size=img_height, patch_size=patch_size, in_channels=img_channels,
                                 embedding_dim=encoded_dim, num_heads=num_heads, num_layers=num_layers,
                                 dropout=dropout)
        model = model.to(device)
    ...
    for epoch in range(num_epochs):
        ...
        writer.add_scalar("TNT_{}_{}'s value_loss".format(envir, encoded_dim), test_loss, epoch)
        writer.add_scalar("TNT_{}_{}'s rho".format(envir, encoded_dim), rho, epoch)
